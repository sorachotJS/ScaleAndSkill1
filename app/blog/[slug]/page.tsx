...
import { createClient } from '@supabase/supabase-js'; // ใช้ client มาตรฐานสำหรับ Server Side
import BlogPostContent from '@/app/components/BlogPostContent'; // 👈 นำเข้าไฟล์ที่เราเพิ่งสร้าง
import { Metadata } from 'next';
import Link from 'next/link';

// สร้าง Supabase Client สำหรับฝั่ง Server (Public Access)
const supabase = createClient(
...
// 1. GENERATE METADATA (หัวใจสำคัญของ SEO และ Facebook Share)
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: post.title,
    description: post.excerpt || 'Read this amazing article on Scale & Skill',
    openGraph: {
      title: post.title,
      description: post.excerpt,
      images: post.cover_image ? [post.cover_image] : [], // รูปที่จะโชว์บน Facebook
      type: 'article',
      publishedTime: post.published_at,
      authors: [post.meta_data?.author || 'Scale & Skill'],
    },
  };
}
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // ส่งข้อมูล Post ทั้งก้อนไปให้ Client Component แสดงผล
